...
| Experiments | Missing success/kill conditions, overdue results, stalled tests, new results and contradictory interpretations | [experiment-workflow](../experiment-workflow/SKILL.md); launch and material conclusions retain human gates. Staging an unlaunched instrument (draft study, unissued participant link) is design work rather than launch, and is permitted where the provider supports that state **and** the instrument can run the stated method — verify both, and stage nothing that cannot answer the question. A fully-designed experiment sitting untouched is a stalled test: report it, never as `healthy` |
