...
**Copy Test Design**
```
The team can't agree on this wording/copy/small UI decision: [describe the options
under debate]. Instead of resolving it through more discussion, design a same-day
copy test: the variants to ship, where real users will see them, the metric that
decides the winner, and the minimum traffic or sample needed to trust the result.
```
